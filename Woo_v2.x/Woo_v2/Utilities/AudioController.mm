...
-(AVAudioSession*)getCurrentAVAudioSession
{
...
- (void)handleRouteChange:(NSNotification *)notification
{
    ...
    NSLog(@"Route change:%@",routeDescription);
    switch (reasonValue) {
...
{
    NSError *error = nil;
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"ringtone" ofType:@"mp3"];
    NSURL *fileURL = [NSURL fileURLWithPath:filePath];
    
    audioFilePlayer = [[AVAudioPlayer alloc]initWithContentsOfURL:fileURL error:&error];
    audioFilePlayer.numberOfLoops = -1;
    audioFilePlayer.volume = 1;
    
}
-(void)startPlayingCallerTune
{
    NSError *error = nil;
    AVAudioSession *session = [AVAudioSession sharedInstance];
    [session setActive:YES error:&error];
    if (nil != error) NSLog(@"AVAudioSession set active failed with error: %@", error);
    NSString *portType = [[[[session currentRoute] outputs] firstObject] portType];
    if (portType &&  portType == AVAudioSessionPortBuiltInSpeaker)
    {
...
-(void)playRingtone
{
    ...
    if (portType &&  portType == AVAudioSessionPortBuiltInReceiver)
    {
