...
+(void)updateBoostData:(NSDictionary *)visitorDict ForWooId:(NSString*)wooId
{
    ...
    if (privateManagedObjectContext)
    {
        [privateManagedObjectContext performBlock:^{
            ...
            BoostDashboard *boostObj = [[self getBoostDataForVisitorId:[visitorDict objectForKey:@"wooUserId"]]firstObject];
...
+(void)insertOrUpdateBoostDashboardData:(NSArray *)boostDataArray withInsertionCompletionHandler:(InsertionCompletionHandler)insertionCompletion{
        ...
        [privateManagedObjectContext performBlock:^{
            ...
            if (boostDataArray && [boostDataArray count] > 0 )
            {
                ...
                for (NSDictionary *visitorDict in boostDataArray) {
                    ...
                    if (!boostObj) {
                        boostObj = (BoostDashboard *)[NSEntityDescription insertNewObjectForEntityForName:@"BoostDashboard" inManagedObjectContext:privateManagedObjectContext];
                        newRecordsAdded = YES;
...
+(NSArray *)getAllBoostProfiles{
    ...
    NSEntityDescription *entityDescription = [NSEntityDescription entityForName:@"BoostDashboard" inManagedObjectContext:managedObjectContext];
    ...
    if (privateManagedObjectContext) {
        ...
        [privateManagedObjectContext performBlock:^{
            ...
            for (NSString *idToBeDeleted in idsToBeDeleted) {
                ...
                if (toBeDeletedCrush)
                {
                    for (BoostDashboard *boostProfile in toBeDeletedCrush)
                    {
...
+(BOOL)isLastProfileBoosted:(NSDictionary *)firstObject{
    ...
    if (allProfile && [allProfile count] > 0) {
        BoostDashboard *lastProfile = [allProfile lastObject];
...
+(NSMutableDictionary *)getGroupedVisitorDataAccordingToTheGroups
{
    ...
    for (BoostDashboard *boost in boostDataArray)
    {
...
+(NSArray *)getAllBoostProfilesSortedByTimestamp
{
    ...
    NSPredicate *duplicatePredicate = [NSPredicate predicateWithBlock: ^BOOL(id obj, NSDictionary *bind) {
        BoostDashboard *e = (BoostDashboard*)obj;
...
+(void)deleteAllVisotorsPresentInSystemForMoreThanDays:(int)numberOfDays{
    ...
    __block NSError *error = nil;
        ...
        [privateManagedObjectContext performBlock:^{
            ...
            NSEntityDescription *entityDescription = [NSEntityDescription entityForName:@"BoostDashboard" inManagedObjectContext:privateManagedObjectContext];
            ...
            for (BoostDashboard *visitorToBeDeleted in selectedBoostArray) {
