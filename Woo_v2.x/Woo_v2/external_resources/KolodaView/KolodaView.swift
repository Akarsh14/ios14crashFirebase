...
private let backgroundCardsTopMargin: CGFloat = 0.0
...
public extension KolodaViewDelegate {
    ...
    func koloda(_ koloda: KolodaView, allowedDirectionsForIndex index: UInt) -> [SwipeResultDirection] { return [.Left, .Right, .Up, .TopLeft, .TopRight] }
    func koloda(_ koloda: KolodaView, didSwipeCardAtIndex index: UInt, inDirection direction: SwipeResultDirection) {}
    func kolodaDidRunOutOfCards(_ koloda: KolodaView) {}
    func koloda(_ koloda: KolodaView, didSelectCardAtIndex index: UInt) {}
...
@objc open class KolodaView: UIView, DraggableCardDelegate {
    ...
    open var needToStopTranslationAndRotationForKoloda:Bool = false
    
    open var crushIsBeingSent:Bool = false
    ...
    fileprivate func layoutCard(_ card: DraggableCardView, atIndex index: UInt) {
        ...
        if index == 0 {
            ...
            if SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(version: "11.0"){
                card.frame = cardParametersFrame
            }
            else{
    ...
    open func frameForCardAtIndex(_ index: UInt) -> CGRect {
       ...
       // let previousCardFrame = index > 0 ? frameForCardAtIndex(max(index - 1, 0)) : CGRectZero
        let yOffset = -(8 * Int(index))
        
        let frame = CGRect(x: xOffset, y: CGFloat(yOffset), width: width, height: height)
    ...
    func card(_ card: DraggableCardView, wasDraggedWithFinishPercentage percentage: CGFloat, inDirection direction: SwipeResultDirection, withGesture gestureRecognizerState:UIPanGestureRecognizer) {
        ...
        card.needToStopTranslationAndRotationForDraggableView = self.needToStopTranslationAndRotationForKoloda
        card.sendingCrush = self.crushIsBeingSent
        
        if let shouldMove = delegate?.kolodaShouldMoveBackgroundCard(self) , shouldMove {
            self.moveOtherCardsWithPercentage(percentage)
        }
        delegate?.koloda(self, draggedCardWithPercentage: percentage, inDirection: direction, withGesture: gestureRecognizerState)
    }
    ...
    func card(cardAllowedDirections card: DraggableCardView) -> [SwipeResultDirection] {
        if let draggableCardIndex = visibleCards.index(of: card) {
            let index = currentCardIndex + draggableCardIndex
            return delegate?.koloda(self, allowedDirectionsForIndex: UInt(index)) ?? [.Left, .Right, .Up, .TopLeft, .TopRight]
        }
        else{
            return [.Left, .Right, .Up, .TopLeft, .TopRight]
        }
    }
    ...
    fileprivate func swipedAction(_ direction: SwipeResultDirection) {
        ...
        print("currentCardSwipedIndex=\(currentCardIndex)")
        currentCardIndex += 1
        ...
        if !visibleCards.isEmpty {
            animateCardsAfterLoadingWithCompletion { [weak self] in
                ...
                if let localDelegate = _self.delegate{
                    print("currentIndex=\(_self.currentCardIndex)")
                    if _self.currentCardIndex >= 0{
                        if _self.currentCardIndex > 0{
                localDelegate.koloda(_self, didSwipeCardAtIndex: UInt(_self.currentCardIndex - 1), inDirection: direction)
                        }
                localDelegate.koloda(_self, didShowCardAtIndex: UInt(_self.currentCardIndex))
                    }else{
                        self!.resetPositionOfCurrentCard()
                    }
                }
            }
        } else {
    ...
    fileprivate func animateCardsAfterLoadingWithCompletion(_ completion: (() -> Void)? = nil) {
        for (index, currentCard) in visibleCards.enumerated() {
            ...
            let cardParameters = backgroundCardParametersForFrame(frameForCardAtIndex(UInt(index)))
            let cardParametersFrame = frameForCardAtIndex(UInt(index))
            var cardFrame = cardParameters.frame
            if SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(version: "11.0"){
                cardFrame = cardParametersFrame
            }
    ...
    open func resetPositionOfCurrentCard(){
    ...
    open func swipe(_ direction: SwipeResultDirection) {
        ...
        let _ = delegate?.koloda(self, shouldDragCardAtIndex: UInt(currentCardIndex))
        ...
        //if !animating {
            if let frontCard = visibleCards.first {
                ...
                frontCard.sendingCrush = self.crushIsBeingSent
                frontCard.isUsedInTagSearch = self.isUsedInTagSearch
    ...
    open func insertCardAtIndexRange(_ indexRange: CountableRange<Int>, animated: Bool = true) {
    ...
    fileprivate func proceedDeletionInRange(_ range: CountableRange<Int>) {
