...
protocol DraggableCardDelegate: class {
    ...
    func card(_ card: DraggableCardView, wasDraggedWithFinishPercentage percentage: CGFloat, inDirection direction: SwipeResultDirection, withGesture gestureRecognizerState:UIPanGestureRecognizer)
...
private let rotationMax: CGFloat = 0.8
private let defaultRotationAngle = CGFloat(Double.pi/4) / 10.0
private let scaleMin: CGFloat = 1.0
public let cardSwipeActionAnimationDuration: TimeInterval  = 0.0
public var cardSwipeActionAnimationDurationForButtonTap: TimeInterval  = 0.8
...
open class DraggableCardView: UIView, UIGestureRecognizerDelegate {
    ...
    open var dragBegin = false
    fileprivate var dragDistance = CGPoint.zero
    fileprivate var swipePercentageMargin: CGFloat = 0.0
    
    open var needToStopTranslationAndRotationForDraggableView:Bool = false
    
    open var sendingCrush:Bool = false
    
    open var isUsedInTagSearch:Bool = false
    ...
    @objc func panGestureRecognized(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            if let dragDirection = dragDirection {
                //100% - for proportion
                delegate?.card(self, wasDraggedWithFinishPercentage: min(abs(100 * percentage), 100), inDirection: dragDirection, withGesture: gestureRecognizer)
            }
        ...
        case .changed:
            ...
            if needToStopTranslationAndRotationForDraggableView == false {
    ...
    fileprivate var directions: [SwipeResultDirection] {
        return delegate?.card(cardAllowedDirections: self) ?? [.Left, .Right, .Up, .TopLeft, .TopRight]
    }
    ...
    fileprivate func updateOverlayWithFinishPercent(_ percent: CGFloat, direction: SwipeResultDirection?) {
        ...
        overlayView?.overlayStrength = 1.0
    }
    ...
    fileprivate func swipeAction(_ direction: SwipeResultDirection) {
        ...
        if self.subviews.count > 0 && (self.subviews.first?.isKind(of: ProfileDeckView.self))! {// check only if subbview is profileDeck card
            if DiscoverProfileCollection.sharedInstance.collectionMode != CollectionMode.my_PROFILE { // dont check for my profile
            ...
            if direction == .Left || direction == .TopLeft || direction == .BottomLeft{
                if UserDefaults.standard.bool(forKey: "firstTimeDisliked") == false && DiscoverProfileCollection.sharedInstance.collectionMode == CollectionMode.discover{
                    resetViewPositionAndTransformations()
                    return
                }
            }
            else if direction == .Right || direction == .TopRight || direction == .BottomRight{
                if UserDefaults.standard.bool(forKey: "firstTimeLiked") == false && DiscoverProfileCollection.sharedInstance.collectionMode == CollectionMode.discover{
                    resetViewPositionAndTransformations()
                    return
                }
            }
        ...
        DispatchQueue.main.async(execute: {
            ...
            translationAnimation?.fromValue = NSValue(cgPoint: POPLayerGetTranslationXY(self.layer))
            translationAnimation?.toValue = NSValue(cgPoint: self.animationPointForDirection(direction))
            translationAnimation?.completionBlock = { _, _ in
                self.removeFromSuperview()
            }
            self.layer.pop_add(translationAnimation, forKey: "swipeTranslationAnimation")
        })

    }
    
    open func resetViewPositionAndTransformations() {
    ...
    func swipe(_ direction: SwipeResultDirection) {
        ...
        if isUsedInTagSearch{
            cardSwipeActionAnimationDurationForButtonTap = 1.2
        }
      ...
      //  if !dragBegin {
            ...
            print("swipe is happening")
            delegate?.card(self, wasSwipedInDirection: direction)
            
            
            DispatchQueue.main.async(execute: {
                let swipePositionAnimation = POPBasicAnimation(propertyNamed: kPOPLayerTranslationXY)
                swipePositionAnimation?.fromValue = NSValue(cgPoint:POPLayerGetTranslationXY(self.layer))
                swipePositionAnimation?.toValue = NSValue(cgPoint:self.animationPointForDirection(direction))
                swipePositionAnimation?.duration = cardSwipeActionAnimationDurationForButtonTap
                swipePositionAnimation?.completionBlock = {
                    (_, _) in
                    self.removeFromSuperview()
                }
                
                self.layer.pop_add(swipePositionAnimation, forKey: "swipePositionAnimation")
                
                let swipeRotationAnimation = POPBasicAnimation(propertyNamed: kPOPLayerRotation)
                swipeRotationAnimation?.fromValue = POPLayerGetRotationZ(self.layer)
                swipeRotationAnimation?.toValue = CGFloat(self.animationRotationForDirection(direction))
                swipeRotationAnimation?.duration = cardSwipeActionAnimationDurationForButtonTap
                
                self.layer.pop_add(swipeRotationAnimation, forKey: "swipeRotationAnimation")
                
                self.overlayView?.overlayState = direction
                let overlayAlphaAnimation = POPBasicAnimation(propertyNamed: kPOPViewAlpha)
                overlayAlphaAnimation?.toValue = 1.0
                overlayAlphaAnimation?.duration = cardSwipeActionAnimationDurationForButtonTap
                self.overlayView?.pop_add(overlayAlphaAnimation, forKey: "swipeOverlayAnimation")
