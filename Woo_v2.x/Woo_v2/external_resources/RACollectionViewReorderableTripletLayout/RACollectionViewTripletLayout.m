...
- (CGSize)collectionViewContentSize
{
    ...
    for (NSInteger i = 0; i < self.collectionView.numberOfSections; i++) {
        ...
        NSInteger numberOfLines = ceil((CGFloat)[self.collectionView numberOfItemsInSection:i] / 3.f);
        CGFloat lineHeight =  [_largeCellSizeArray[i] CGSizeValue].height +
                (numberOfLines - 1) * ([_smallCellSizeArray[i] CGSizeValue].height + _lineSpacing);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _insets.left = 5.0;
    _insets.right = 5.0;
    CGFloat smallCellSideLength = ((_collectionViewSize.width - (_insets.left + _insets.right)) - (2 * _itemSpacing)) / 3.f;
    CGFloat largeCellSideLength = (_collectionViewSize.width - (_insets.left + _insets.right)) - smallCellSideLength - _itemSpacing;
    ...
    if (line == 0) {
        ...
        CGFloat lineOriginY = _largeCellSize.height + (_smallCellSize.height * (line - 1)) + sectionHeight + lineSpaceForIndexPath + _insets.top;
        CGFloat cellWidthWithLineSpacing = _smallCellSize.width + _lineSpacing;
        if (indexPath.item % 3 == 0) {
            attribute.frame = CGRectMake(_insets.left, lineOriginY, _smallCellSize.width, _smallCellSize.height);
        }
        else if (indexPath.item % 3 == 1){
            attribute.frame = CGRectMake(_insets.left + cellWidthWithLineSpacing, lineOriginY, _smallCellSize.width, _smallCellSize.height);
        }
        else{
            attribute.frame = CGRectMake(_insets.left + 2*cellWidthWithLineSpacing, lineOriginY, _smallCellSize.width, _smallCellSize.height);
        }
    }
