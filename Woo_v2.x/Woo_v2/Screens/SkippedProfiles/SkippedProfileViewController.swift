//
//  SkippedProfileViewController.swift
//  Woo_v2
//
...
class SkippedProfileViewController: BaseClassViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

    @IBOutlet weak var skippedProfileCollectionView: UICollectionView!
    ...
    var skippedProfileArray: NSMutableArray = []
    ...
    var btnFromDiscover : Bool = false
    ...
    var currentSkippedUserDetail:SkippedProfiles?
    ...
    override func viewDidLoad() {
        ...
        if (LoginModel.sharedInstance()?.gender == "FEMALE"){
            UserDefaults.standard.set(true, forKey: "hasVisitedSkippedSection")
    ...
    override func viewWillAppear(_ animated: Bool) {
       ...
       if isPaidUser == true
       {
        ...
        AppLaunchModel.sharedInstance().numberOfTimesSkippedProfileLaunched += 1
        if AppLaunchModel.sharedInstance().numberOfTimesSkippedProfileLaunched > 10 {
            AppLaunchModel.sharedInstance().numberOfTimesSkippedProfileLaunched = 1
        }
       if isPaidUser
        {
            AppLaunchModel.sharedInstance().isNewDataPresentInSkippedSection = false
            NotificationCenter.default.addObserver(self, selector: #selector(showLoadingForPagination), name: NSNotification.Name(rawValue: kPaginationCallStartedSkippedProfiles), object: nil)
            
            NotificationCenter.default.addObserver(self, selector: #selector(showLoadingForPagination), name: NSNotification.Name(rawValue: kPaginationCallStoppedSkippedProfiles), object: nil)
            
            NotificationCenter.default.addObserver(self, selector: #selector(reloadViewMaintainingScrollPosition), name: NSNotification.Name(rawValue: kNewSkippedProfilesAddedOnPagination), object: nil)

        }
        if (scrolledCollectionView > 0){
        skippedProfileCollectionView.setContentOffset(CGPoint(x: 0,y :scrolledCollectionView), animated: false)
        }
        }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: kPaginationCallStartedSkippedProfiles), object: nil)
        
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: kPaginationCallStoppedSkippedProfiles), object: nil)
        
        NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: kNewSkippedProfilesAddedOnPagination), object: nil)
      }
...
//        }
        ...
        if WooPlusModel.sharedInstance().isExpired {
            ...
            if(SkippedProfileAPIClass.getIsfetchingSkippedDataFromServerValue())
            {
                ...
                if(loadingMoreViewHeightConstraint.constant != 55.0)
                {
                    ...
                    skippedProfileCollectionView.reloadData()
                    if skippedProfileCollectionView.contentOffset.y > 0
                    {
    ...
    {
        if SkippedProfiles.getAllSkippedProfiles().count <= 0
        {
            return
        }
        ...
        if (LoginModel.sharedInstance()?.gender == "FEMALE")
        {
            ...
            let arrayOfAddedSkippedProfiles = notification.object as! [SkippedProfiles]
            ...
            let indexForInsertion = Int(SkippedProfiles.getExpiryIndexForInsertionForSkipped())
                ...
                DispatchQueue.main.async {
                    ...
                    self.expiryView?.expiredCollectionView.performBatchUpdates(
                        ...
                        if (completed)
                        {
                            ...
                            self.skippedProfileCollectionView.reloadData()
    ...
    func getExpiryProfilesFromArray(addedProfiles:[SkippedProfiles]) -> [SkippedProfiles]
    {
        let utilities = Utilities.sharedUtility() as! Utilities
    ...
    func addAndShowNewProfilesView()
    {
        if seeNewProfilesView == nil
        {
            ...
            seeNewProfilesView?.seeProfilesButtonHandler =
                {
                    self.skippedProfileCollectionView.setContentOffset(CGPoint(x: 0.0, y: -CGFloat((self.expiryView?.bounds.size.height) ?? 0)) , animated: true)
    ...
    @IBAction func moreInfoButtonTapped(_ sender: UIButton){
        ...
        if WooPlusModel.sharedInstance().isExpired {
            ...
            print(skippedProfileArray.count)
            
            if skippedProfileArray.count < 1 && btnFromDiscover == true {
                // got to discover
                self.navigationController?.popViewController(animated: true)
                ...
                var funnelString = "Skipped_locked_WP"
                
                if sender.tag == 200 || sender.tag == 300{
                    funnelString = "Skipped_WP_cta"
                }
    ...
    func afterPurchaseWork(){
        ...
        self.skippedProfileCollectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
        self.skippedProfileCollectionView.contentOffset = CGPoint(x: 0, y: 0)
    ...
    override func awakeFromNib() {
        ...
        self.navBar?.setTitleText(NSLocalizedString("Skipped Profiles", comment: "Skipped Profiles"))
    ...
    func updateSkippedProfileData(){
        ...
        if (LoginModel.sharedInstance()?.gender == "MALE"){
            ...
            if isPaidUser{
                skippedProfileArray.addObjects(from: SkippedProfiles.getAllSkippedProfiles())
            }
            else{
                if WooPlusModel.sharedInstance().showExpiryEnabledForSkippedProfiles {
                    skippedProfileArray.addObjects(from: SkippedProfiles.getAllNonExpiringSkippedProfiles())
                    if SkippedProfiles.getAllExpiringSKippedProfiles().count > 0 {
                            if expiryView == nil
                            {
                                if skippedProfileCollectionView.contentOffset.y <= 0{
                                    addExpiredViewNow()
                                    expiryView?.expiredDataArray = updateExpiryArrayForExpiredData(SkippedProfiles.getAllExpiringSKippedProfiles() as NSArray, type: .SkippedProfiles) as [AnyObject]
                                ...
                                self.skippedProfileCollectionView.contentInset = UIEdgeInsets(top: CGFloat((expiryView?.frame.size.height)!), left: 0, bottom: 0, right: 0)
                                self.skippedProfileCollectionView.contentOffset = CGPoint(x: 0, y: -Int((expiryView?.frame.size.height)!))
                        }
                    }
                }
                else{
        ...
        if SkippedProfiles.getAllSkippedProfiles().count > 0 {
            if showExpiryPopUp{
                if UserDefaults.standard.bool(forKey: "ProfileCanExpireHasBeenShowed") == false {
                    self.skippedProfileCollectionView.isUserInteractionEnabled = false
                    if profileCanExpireView == nil {
                        ...
                        profileCanExpireView?.dismissHandler = {
                            UIView.animate(withDuration: 0.2, animations: {
                                ...
                                self.skippedProfileCollectionView.isUserInteractionEnabled = true
                                if self.meContainerView != nil{
...
//                        }
    ...
    func addExpiredViewNow(){
        ...
        if expiryView == nil
        {
                ...
                expiryView?.purchaseHandler = {() in
                    self.btnFromDiscover = false
                    self.moreInfoButtonTapped(UIButton())
                }
            self.skippedProfileCollectionView.contentInset = UIEdgeInsets(top: CGFloat(expiryCellHeight), left: 0, bottom: 0, right: 0)
                    self.skippedProfileCollectionView.contentOffset = CGPoint(x: 0, y: -expiryCellHeight)
                self.skippedProfileCollectionView.addSubview(expiryView!)
            self.skippedProfileCollectionView.sendSubviewToBack(expiryView!)
                
                expiryView?.snp.makeConstraints { (make) in
                    make.top.equalTo(self.skippedProfileCollectionView).offset(-expiryCellHeight)
                    make.leading.equalTo(self.skippedProfileCollectionView).offset(0)
                    make.trailing.equalTo(self.skippedProfileCollectionView).offset(0)
                    make.height.equalTo(expiryCellHeight)
                    make.width.equalTo(SCREEN_WIDTH)
                }
        }
        expiryView?.dataType = .SkippedProfiles
        ...
        expiryView?.openExpiryVcHandler = {(dataType:MeDataType) in
            ...
            expiryCollectionVc.dismissViewControllerHandler = { (expiryToBeReloaded:Bool) in
                ...
                {
                    let expiryDataArray = updateExpiryArrayForExpiredData(SkippedProfiles.getAllExpiringSKippedProfiles() as NSArray, type: .SkippedProfiles) as [AnyObject]
        ...
        expiryView?.openProfileDetailHandler = { (needToTakeAction:PerformAction, userProfile:AnyObject, comingFromExpired:Bool, expiredIndexPath:IndexPath) in
            ...
            self.currentSkippedUserDetail = userProfile as? SkippedProfiles
    ...
    func showOrRemoveSellingMessages() -> Void {
        ...
        if WooPlusModel.sharedInstance().isExpired {
            ...
            if WooPlusModel.sharedInstance().showExpiryEnabledForSkippedProfiles{
                totalCount = totalCount + SkippedProfiles.getAllExpiringSKippedProfiles().count
            }
            if totalCount < 1 {
                showSellingMessage = false
                meSectionEmptyViewObj?.isHidden = false
                skippedProfileCollectionView.isHidden = true
                topToCollectionViewConstraint.constant = 44.0
            }
            else{
                showSellingMessage = true
                meSectionEmptyViewObj?.isHidden = true
                skippedProfileCollectionView.isHidden = false
            }
        }
        else{
            if skippedProfileArray.count + SkippedProfiles.getAllExpiringSKippedProfiles().count < 1 {
                showSellingMessage = false
                if SkippedProfileAPIClass.getIsfetchingSkippedDataFromServerValue(){
                    meSectionEmptyViewObj?.isHidden = true
                    skippedProfileCollectionView.isHidden = false
    ...
    func toggleSellingMessage() -> Void {
        ...
        if WooPlusModel.sharedInstance().isExpired {
            if skippedProfileArray.count < 1 {
                ...
                meSectionEmptyViewObj?.setEmptyScreenDetail(withTitle: NSLocalizedString("People who you skip show here", comment: ""), sellingMessage: AppLaunchModel.sharedInstance().discoverMoreProfileText, actionButtonTitle: NSLocalizedString("DISCOVER PEOPLE", comment: ""), showBoostIcon: false, isUserMale: Utilities().isGenderMale(LoginModel.sharedInstance()?.gender))
           ...
           // skippedProfileCollectionView.isScrollEnabled = false
//            skippedProfileCollectionView.isUserInteractionEnabled = false
            isPaidUser = false
        }
        else{
//            if skippedProfileArray.count < 1 {
            
            meSectionEmptyViewObj?.setEmptyScreenDetail(withTitle: NSLocalizedString("All profiles you skip will show here", comment: ""), sellingMessage: NSLocalizedString("Sometimes giving a second glance is important.\nGet started by reviewing more profiles.", comment: ""), actionButtonTitle: NSLocalizedString("DISCOVER PROFILES", comment: ""), showBoostIcon: false, isUserMale: Utilities().isGenderMale(LoginModel.sharedInstance()?.gender))
            meSectionEmptyViewObj?.showActionButton(showActionButton: false)
            
//            } 
            skippedProfileCollectionView.isScrollEnabled = true
//            skippedProfileCollectionView.isUserInteractionEnabled = true
            isPaidUser = true
        }
        skippedProfileCollectionView.isScrollEnabled = true
    ...
    func performActionForType(_ actionType:PerformAction,indexPath:IndexPath){
        ...
        case .Like:
            if  comingFromProfile == true{
                ...
                profileAction.likeActionPerformed(.SkippedProfiles, userObject: currentSkippedUserDetail! as AnyObject)
            }
            break
        ...
        case .Pass:
            if  comingFromProfile == true{
                ...
                profileAction.dislikeActionPerformed(.SkippedProfiles, userObject: currentSkippedUserDetail! as AnyObject)
            }
            break
        ...
        case .CrushSent:
            if  comingFromProfile == true{
                ...
                profileAction.crushActionPerformed(.SkippedProfiles, userObject: currentSkippedUserDetail! as AnyObject)
            }
            break
    ...
    func reloadViewAfterRemovingData(_ indexPath:IndexPath) {
        ...
        if (self.skippedProfileArray.count == 0){
            self.skippedProfileCollectionView.deleteSections(IndexSet(integer: indexPath.section))
        }
        else{
            if let _ = self.skippedProfileCollectionView.cellForItem(at: indexPath){
                self.skippedProfileCollectionView.deleteItems(at: [indexPath])
        ...
        /*
        self.skippedProfileCollectionView.performBatchUpdates({
    ...
    */
        ...
        case UICollectionView.elementKindSectionHeader:
            //3
                ...
                if LoginModel.sharedInstance()?.gender == "FEMALE" || !isPaidUser{
                    headerView.setHeaderTextForSection(isSectionForBoosted: false, textForHeader: NSLocalizedString("Recently Skipped (expiring soon)", comment: ""))
                }
                else{
                    headerView.setHeaderTextForSection(isSectionForBoosted: false, textForHeader: NSLocalizedString("Recently Skipped", comment: ""))
                }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        var skippedProfileDetailDetail:SkippedProfiles? = skippedProfileArray.object(at: (indexPath as NSIndexPath).row) as? SkippedProfiles
        if skippedProfileDetailDetail == nil{
            skippedProfileDetailDetail = SkippedProfiles()
        }
        cell.setUserDetail(userDetail: skippedProfileDetailDetail!, isBundledProfile: false, forType: .SkippedProfiles, isPaidUser: isPaidUser, showLock: WooPlusModel.sharedInstance().maskingEnabledForSkippedProfiles, noOfTotalProfiles: skippedProfileArray.count, isExpired: false, profileIndex: indexPath.row, usedInExpiryProfiles: false)
        ...
        cell.meProfileDeckView?.dismissHandler = { (needToTakeAction:PerformAction, userProfile:AnyObject) in
            ...
            if needToTakeAction == PerformAction.Pass {
                ...
                WooScreenManager.sharedInstance.oHomeViewController?.crushFunnelMessage = "Skipped_profile_crush_tap"
                self.needToOpenCrushSendViewInProfileDetail = true
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        // Srwve Event
        if isPaidUser {
            if (skippedProfileCollectionView.indexPathsForSelectedItems?.first) != nil{
                scrolledCollectionView = Int(skippedProfileCollectionView.contentOffset.y)
            }
            ...
            if Utilities().reachable() == true {
                if let visitorDetail:SkippedProfiles = skippedProfileArray.object(at: indexPath.row) as? SkippedProfiles{
    ...
    func pushtoDetailView(_ skippedProfile:SkippedProfiles, indexPath:IndexPath){
            ...
            if skippedProfile.userWooId == (UserDefaults.standard.object(forKey: kWooUserId) as! String) {
                ...
                if model.wooId == nil{
                    ...
                    if self.actionPerformedFromExpire{
                        ...
                        cell = self.skippedProfileCollectionView.cellForItem(at: indexPath) as? MeCollectionViewCell
                    }
                ...
                DispatchQueue.main.async(execute: {
                    self.performSegue(withIdentifier: kPushtFromSkippedProfileToDetailProfileView, sender: model)
                })
            }
        })
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == kPushtFromSkippedProfileToDetailProfileView){
...
//            let chatViewNavControllerObj: UINavigationController = segue.destination as! UINavigationController
   ...
   @objc func showCrushOverlayAndUpdateView(){
    if (indexPathToDelete != nil) {
        let cell: MeCollectionViewCell? = skippedProfileCollectionView.cellForItem(at: indexPathToDelete!) as? MeCollectionViewCell
        cell?.meProfileDeckView?.showOverlayForCrushSent()
        }
    }
}

extension SkippedProfileViewController : UIScrollViewDelegate{
