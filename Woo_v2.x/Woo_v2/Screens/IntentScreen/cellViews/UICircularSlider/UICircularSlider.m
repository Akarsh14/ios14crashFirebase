...
@property(nonatomic, assign) BOOL onlyNotifyWhenKnobTouched;
...
#define kThumbRadius SCREEN_WIDTH*0.03125
#pragma mark -
...
- (void)setValue:(float)value {
	if (value != _value) {
        ...
        if (self.isContinuous) {
            if (_onlyNotifyWhenKnobTouched == true) {
...
- (void)setValueAndDoNotNotify:(float)value {
    if (value != _value) {
        if (value > self.maximumValue) { value = self.maximumValue; }
        if (value < self.minimumValue) { value = self.minimumValue; }
        _value = value;
...
- (void)setup {
    ...
    self.thumbnailRadius = kThumbRadius;
...
- (CGFloat)sliderRadius {
	...
	radius -= MAX(kLineWidth, self.thumbnailRadius);
	return radius;
}
- (void)drawThumbAtPoint:(CGPoint)sliderButtonCenterPoint inContext:(CGContextRef)context {
	...
	CGContextAddArc(context, sliderButtonCenterPoint.x, sliderButtonCenterPoint.y, self.thumbnailRadius, 0.0, 2*M_PI, NO);
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat radiusForKnob = [self sliderRadius]-(self.thumbnailRadius/2.0);
	switch (self.sliderStyle) {
		...
		default:
			...
			[self drawCircularTrack:self.maximumValue atPoint:middlePoint withRadius:radius inContext:context];
			[self.minimumTrackTintColor setStroke];
			self.thumbCenterPoint = [self drawCircularTrack:self.value atPoint:middlePoint withRadius:radiusForKnob inContext:context];
			break;
	}
...
#pragma mark - Thumb management methods
- (BOOL)isPointInThumb:(CGPoint)point {
	CGRect thumbTouchRect = CGRectMake(self.thumbCenterPoint.x - self.thumbnailRadius, self.thumbCenterPoint.y - self.thumbnailRadius, self.thumbnailRadius*2, self.thumbnailRadius*2);
...
- (void)panGestureHappened:(UIPanGestureRecognizer *)panGestureRecognizer {
    _onlyNotifyWhenKnobTouched = true;
